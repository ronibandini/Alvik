...
batteryLimit=30
alwaysCharge=0
endPathColor1="LIGHT GREY" # Sometimes WHITE is recognized as light grey 
testMode=0
...
try:
    while True:
        ...
        while not alvik.get_touch_cancel():
            ...
            # check battery and change color
            if checkCounter==25:
              if alvik.get_battery_charge()<batteryLimit:
                print("Needs the charger")
                changeLights('red')                
              else:
                print("Battery is ok")
                changeLights('green')
            ...
            if (ir_left<=lineLimit and ir_center<=lineLimit and ir_right<=lineLimit):
                ...
                if (chargingPath==0):
                  ...
                  if (getColor()==endPathColor1):  
                    chargeAlvik()
                  else: 
                    print("Missed the line. Search where the path is")
                    searchPath()  
                    chargingStationPath()
            else:
